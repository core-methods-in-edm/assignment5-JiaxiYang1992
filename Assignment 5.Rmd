...
```{r}
levels(D2$prediction)
D2$advice <- ifelse(D2$score <=0.4, "intervene", ifelse(D2$score > 0.4 & D2$score <=0.8, "monitor", "no action"))
sum(D2$prediction=="monitor")
sum(D2$prediction=="intervene")

# We found that there are 84 incorrect predictions with the tree methods. They were misjudged from "no action" to "monitor".
